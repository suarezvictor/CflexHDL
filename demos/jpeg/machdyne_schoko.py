...
class BaseSoC(SoCCore):
    ...
    def __init__(self, revision="v1", device="45F", sdram_rate="1:2",
        ...
        self.add_video_framebuffer(phy=self.videophy, timings="640x480@60Hz",
            clock_domain="video", format="rgb888")

        # Leds -------------------------------------------------------------------------------------
        if with_led_chaser:
            self.leds = LedChaser(
                pads         = platform.request("rgb_led"),
...
def main():
    ...
    parser.add_target_argument("--cable",           default="ft2232", help="Specify an openFPGALoader cable.")
